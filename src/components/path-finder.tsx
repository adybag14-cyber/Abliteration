...
import { ArrowRight, Cpu, Eye, Gauge, Layers3, Sparkles, Terminal, Undo2 } from "lucide-react";
...
export function PathFinder() {
  const [hardware, setHardware] = useState<Hardware>("none");
  ...
  const recommendation = useMemo(() => {
    if (hardware === "none") return { title: "Hour 0 · C++26 start", reason: "Ten minutes. No GPU, no Python. Downloads live in the Lab strip (#lab). Unpack one unique nightly and run guide → doctor → limits → self-check → demo. This is the geometry lab — not a real-model edit.", profile: "Hour 0 · no GPU", doc: "docs/cxx26-researcher-guide.md", icon: Terminal };
